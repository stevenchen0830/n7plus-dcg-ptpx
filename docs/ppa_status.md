...
3. 两级兜底（都不改 REG_IN/REG_OUT，不改吞吐）：
   - 流程级：`DCG_RETIME_MAC=1`，`run_dcg.tcl` 把除 MAC 流水寄存器外的所有寄存器
     ...
     不改 RTL、不重跑回归、面积基本不变。
   - RTL 级：`mac4` 分支，每个 10×8 乘法拆成两个 10×4 半乘先寄存，MAC 四级流水。
     ASAP7 慢角综合级 A/B（[docs/asap7_ab.md](asap7_ab.md)）：最差路径 1,308 → 983 ps
     （−25 %），MAC1 降到 810 ps，新瓶颈是旋转器 A 级 983 ps；面积 +24 %
     （43,172 → 53,477 µm²），触发器 +22.8 k，+1 拍延时，面积项估计少约 4 分。
     只有当负 slack 造成的损失大于这 4 分（约 slack < −200 ps）时才值得切换。
