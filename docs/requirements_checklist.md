...
| # | 要求 | 判定 | 依据 |
| --- | --- | --- | --- |
| 2.1 | RGBA，每像素 40 bit，四分量各 10 bit，PPC = 4 | 符合（已验证） | 160 bit 数据口；16 条 10 bit lane 并行（`G_LANE`，`img_filter.v:624`）；所有回归按 4 分量 × 4 像素逐分量比对 |
...
| 2.4 | `blk_h` = 1，`blk_v` = 1～49 奇数，可超出图像高度 | 符合（已验证） | `verification/logs/all_blkv.log`：25 个合法值各 2 帧（50 帧 / 233,024 次比对 / 0 错误），覆盖掩码 `2AAAAAAAAAAAA`；H = 24 帧对 `blk_v` ≥ 25 即“超出高度”；此前缺失的 27/31/37/45 另有定向 8 帧 `blkv_gap.log` |
...
| 2.11 | `(Σcoef·pix + 64) >> 7`，>1023 饱和为 1023 | 符合（已验证） | `img_filter.v:664-667`；每个输出分量与金标准逐一比对 |
...
**mac4 分支有两处**。第二处是时序兜底：MAC 从三级改为四级流水（每个 10×8 乘法拆成两个 10×4 半乘先寄存，`pplo_q/pphi_q`，`G_PROD`），+1 拍输出延时，其余接口与协议不变。第一处与 `main` 相同：`rem_q` / `rem_f` 原来在 `frm_start` 当拍直接从 `img_height` 端口装载，
